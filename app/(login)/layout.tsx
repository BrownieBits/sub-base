  ...
  return (
    <section className="overflow-y-auto overflow-x-hidden">
      <main className="flex min-h-svh width-full">
        {data.items.length === 0 ? (
          <section className="flex-1 bg-primary p-[30px] hidden md:flex"></section>
        ) : (
          <section
            className="flex-1 bg-primary p-[30px] hidden md:flex bg-cover bg-center"
            style={{
              backgroundImage: `url('https:${data.items[0].fields.desktop.fields.file.url}')`,
            }}
          ></section>
        )}
        <section className="flex flex-1 p-[30px]">{children}</section>
      </main>
    </section>
  );
}
