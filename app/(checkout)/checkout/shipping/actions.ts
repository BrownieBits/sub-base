...
export async function getSelfShipping(ship_to: Address, ship_from: Address, carriers: ShippingCarrier[]) {
    'use server'
    ...
    const rates: Rate[] = resultJSON.map((rate: any) => {
        if (rate.shipping_amount !== null) {
            return {
                ...
                rate: parseFloat(rate.confirmation_amount.amount) + parseFloat(rate.insurance_amount.amount) + parseFloat(rate.requested_comparison_amount.amount) + parseFloat(rate.shipping_amount.amount),
